...
| **v1.5.9** | ✅ done (2026-08-27) | (this commit) | **VSCode 扩展原生 ▶ play 按钮**(替换 v1.5.6-patch2 Code Runner 集成)。`vscode-ext/` 从纯清单扩展升级为带运行时的命令扩展:`commands.jhyy.runFile`(▶)+ `jhyy.compileOnly`(🛠, 普通 title bar 区)+ `menus.editor/title/run` + `Terminal.sendText`(ms-python.python 模式,零 DAP/零 tasks.json)。PATH 探测 (`spawnSync --version`) 兜底 + "打开设置" 通知 affordance。`installer/vscode-ext/package.ps1` 加 `npm install` + `npx tsc` 步骤(`if (Test-Path vscode-ext/src)` gate 兼容旧 manifest-only build);`installer/common/install-vsix.bat` 删 Step 2/3(Code Runner 装 + executorMap 写),`/CONFIGURE_PS1`/`/JHY_DIR` 保留接受(WiX 兼容)但不调用。`configure-coderunner.ps1` 留盘不动(MSI `<File>` 引用,ICE 兼容)。`vscode-ext/icon.svg` + `icon.png` 双 asset;扩展商店图标从无到 mint J(顶 `icon: ./icon.png`)。`package-lock.json` track 用于 build 复现;`vscode-ext/node_modules/` + `vscode-ext/out/` 入 `.gitignore` |
| **v1.5.10** | ✅ done (2026-08-27) | (this commit) | **RunOnce 自动装 VSCode 扩展 — 真"开箱即用"闭环**。`installer/common/install-configure-all.bat` 加第 3 步 inline `where code` + `for %%F in (*.vsix) do code --install-extension "%%F" --force`,非 fatal(找不到 `code` → silent skip + 提示手动命令)。废弃 `install-vsix.bat`(dead code,v1.5.7 设计就跳过,实跑发现 arg parser bug:`%~1` 剥引号后 `/VSIX:path` ≠ `/VSIX` 比对失败 → VSIX 保持空 → exit 255;`code --install-extension` 自身从 CLI shim 跑不需要 VSCode UI 开着)。Burn upgrade v1.5.9 → v1.5.10;MSI Bundle UpgradeCode 同,ProductCode 自动重生成。**用户最后一步体验**:装 MSI → logoff/logon → RunOnce 自动 `code --install-extension`,全程 0 PowerShell 命令,jhyy.exe / qbe.exe / VSCode 扩展全到位。regress 53/53 PASS 不变(本次纯 installer/扩展侧改动) |
