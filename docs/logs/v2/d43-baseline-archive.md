...
| **v2.11.18** | `3f96814870a3afb0d9b59e6d212165ffddeaada5ff4d8b29eda83126c0253cf8` | **当前 active baseline**| codegen.jhyy cg_emit_phi → move-pair lowering (ir_emit_copy_tmp + 新 helper, per W-072 cherry-pick chain + main phi fix). NODE_IF + NODE_MATCH 前驱块末尾 emit copy, merge 直接 return result, QBE ssa() pass 自动构造 SSA. 11 phi 测试 PASS (5/5 ship gate, grep "phi " = 0 全 5). 总 IL 大小变化 (ndeccls 不变, IL 略增 copy 指令, QBE copy() pass 后期消除). |
