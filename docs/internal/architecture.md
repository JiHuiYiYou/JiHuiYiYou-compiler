...
> 流水线、源文件布局、关键设计决策。
>
> **Last updated**: v2.11.18 (2026-09-16) — C 端(`compiler/src/*.c`)**freeze 决策** (2026-09-16): 自 v2.5.0 self-backend 引入后基本冻在 v2.4.0 baseline,只有 build-bootstrap 必前置 (jhyy_stage0.exe SIGSEGV 之类) 才 cherry-pick / 改 (W-068 / W-072 例);新 codegen feature (v2.x 中/末 / v3.x / QBE 自写 / N 代 fixed point) 全走 jhyy 端 (`compiler/src0/*.jhyy`);D43 closure 改为 **jhyy-side internal** (v1.exe → v2.exe → v3.exe → v4.exe → v5.exe .il byte-equal),不再要求 C-side mirror;终态 M5 (v1.x 末 Phase 4) 一次性 `rm src/*.c` + untrack QBE + 删 runtime.c。**v2.0 阶段 (v2.0.0 → v2.4.0) 5 版本全 ship** (tags `v2.3.0` / `v2.4.0`);**v2.7.0 末 3-commit ship + v2.7.1 3-commit ship**(Linux ELF runtime + regress.py `--cross` 实 wire);**v2.8.0 → v2.8.3 M2 ship + W-070 真修 + docker gcc chain 5/5 sysv PASS**;**v2.9.0 V2-C Part 1 ship** (src0 revert + N≥3 selfhost fixed point harness);**v2.11.x series** (v2.11.0-16 self-backend iter on axis-v2 + v2.11.17 audit + v2.11.18 phi 修復 merge → main);v2.11.19 (float QBE_FALLBACK) + v2.11.20 (B-runtime 診斷) planned。**self-backend regress post-merge 96/115 PASS** (v2.11.16 baseline 87/115 → **+9 测试**; +9.5% gain);**D43 closure 当前 active baseline `7bf9c1d4...`** (v2.11.18 axis-v2 merged codegen,实际验证 N=5 byte-equal;v2.9.0/v2.11.0/v2.11.1/v2.11.2/v2.11.3 退役;**注**:main v2.11.18 D43 `3f968148...` 是 main-only codegen 不含 Iter 3/4 的 closure,跟 axis-v2 `7bf9c1d4...` 是不同 active baseline,都是 byte-equal closure,见 [`docs/logs/v2/d43-baseline-archive.md`](../logs/v2/d43-baseline-archive.md) v2.11.18 row)。
