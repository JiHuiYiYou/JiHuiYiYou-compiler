...
| [W-079](#w-079-qbe-amd64_win-mixed-l-w-w-extern-fn-args-bug--i32-return-zero-extension-missing-v323-defer) | ⏳ DEFER (2026-09-26) | QBE amd64_win 2 bugs:(1) mixed (l, w, w) extern fn args uninit registers;(2) main_jhyy 末尾 i32 return 不 zero-extend → bash exit code garbage。**workaround (Phase 1)**:std_io 走 single-type-pointer externs (working QBE path);std_os 走 M0 stubs (input validation);std_os_getenv 走 1-arg pointer-return pattern。**真修**:QBE zero-extend + std_io_print / print_err 走 C-side `jh_stdout_get / jh_stderr_get` getters 或 self-backend (codegen_amd64) 替代 QBE。推到 v3.x mid sprint |
...
| **引用** | `docs/abis/jhyy-lang-spec-stdlib-io-supplement-v3.2.3.md § 5`; `docs/plans/v3/v3.2.3-plan.md § 风险` |
