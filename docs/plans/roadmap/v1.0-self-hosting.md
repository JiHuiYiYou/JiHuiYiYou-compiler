# v1.0: 自举 — 用 jhyy 重写编译器

> 目标：将 C 编译器 1:1 移植到 jhyy 语言，实现编译器编译自身
> 前置：v0.x 完成 (C 编译器可用)
> 产出：自举的 jhyy 编译器 (`jhyy_1`)

> **跟 jhyy_OS 联动**：v1.x 自举闭环达成 = jhyy_OS 启动所需"全栈 jhyy 化"目标的**第一步**达成。v1.x 完成后，jhyy 编译器能编自己，但还**不能编 OS**（缺 P0 特性）。OS 必需的语言特性见 [`v2.0.0-os-prep.md`](../v2/v2.0.0-os-prep.md)；v3.x 实现。**Cap<T> 类型系统草案**在 v1.x 启动阶段就锁定 spec（成本极低），见 [`v3.x-capability-spec.md`](v3.x-capability-spec.md)。
...
### Stage 0 / Stage 1 / Stage 2 三阶段定义

| 阶段 | 验证内容 | 当前状态 | 目标 sprint |
|------|----------|---------|-------------|
| **Stage 0** | `jhyy_v1` (jhyy_0 编出的 jhyy 编译器) 编 `compiler/src0/arena.jhyy` 通过 QBE typecheck + 可运行（exit code 正确） | ✅ **v0.8 commit 12 已达成**（W-008 + W-009 fix 协同让 arena.jhyy emit 通过 typecheck） | — |
| **Stage 1** | `diff jhyy_0.il jhyy_1.il` **byte-equal**（jhyy_0 vs jhyy_1 编同一 .jhyy 产 .il 字节相等） | ✅ **v0.9 wip shipped 2026-08-11**（wip commit 2.83） | — |
| **Stage 2** | `jhyy_1` 编 `compiler/src0/main.jhyy` 跑 regress = **持平 baseline** | ✅ **v1.0.0 TAGGED 2026-08-10**（commit `eabee0d`）;实际 regress = **50/53 PASS**（0 failed, 3 skipped）— 优于原预期"12 OK 持平即可" | — |

**为什么 Stage 0 不够 = Stage 1**：Stage 0 只验证 jhyy_v1 能编 jhyy_v1 自身编自己（compiler compiles compiler compiles compiler）。Stage 1 才验证编出的 .il 跟 jhyy_0 编出的 .il 字节相等 = **fixed point 性质**（编译器自身的输出稳定）。

**为什么 Stage 2 比 Stage 1 更进一步**：Stage 2 在 Stage 1 byte-equal 基础上还验证行为正确（jhyy_1 编 jhyy_1 → 编 fib.jhyy → 跑出 fib(30)=832040 等行为跟 jhyy_0 一致）。

> **真闭环回归预期(2026-08 锁定)**:jhyy_1 编 jhyy_1 跑 regress 大概率 = **12 OK 持平**(剩余 CERR 是 parser 翻译层缺 match-expr / const-array / import,属 sprint 5/6 范畴);**不接受"要求 47/47 全过"作为 v1.0 闭环门槛**。验证时点:commit 13+ 跑 arena_test 端到端。详见 [`architecture-refactor.md` § 1.1.1](architecture-refactor.md)。
>
> **🔄 UPDATE 2026-08-10 (v1.0.0 TAGGED `eabee0d`)**:预测偏低。实际 regress = **50/53 PASS**（0 failed, 3 skipped）— Sprint 4.5 C step 1-4 (commit 2.42-2.44) 修复 len() builtin + float_cmp truncd + match-expr parser 后 baseline 提升至 47/53,Sprint 4.25 A′ sentinel 真修 W-005 #2 family 后 50/53 持平。详见 [`docs/logs/v1/changelog-v1.0.0.md`](../logs/v1/changelog-v1.0.0.md) + [`retrospective-v1.0.0.md`](../logs/v1/retrospective-v1.0.0.md)。

---

## Step 2.0: 确认 v0.x 编译器支持自举所需全部特性

> **本节已被 lang-spec v1.1.0 附录 C 取代**(per CLAUDE.md "v1.1.0 锁定 = v1.0.0 基础 + v0.7 7A/7B 增量")。v1.x 启动条件（lang-spec § 附录 C 行 1132-1137）：
...
> **事实（v0.6 changelog 验证）**：lang-spec 附录 C 行 1100-1113 列的 9 个 C 模块 → JHYY 子集映射**全部 ✓**，即每个 C 编译器模块都能用 v1.0.0 spec 表达。v1.x 启动门槛的语言特性部分**已达成**。
>
> **遗留 P3 限制（lang-spec 附录 B 追更后）**：
> - 变参函数（printf `...`）：JHYY 侧需手动展开，自举可手写 wrapper
> - 函数回调（JHYY 函数指针 → C 调用）：v1.x 考虑
> - 嵌套 import `utils::io`：v0.6+ 候选（v0.6 sprint 6B 未实现，仍 P3）
> - struct/enum 跨 FFI 边界：sprint 6D 部分修 pointer-to-struct，全 ABI 兼容 v1.x 处理
...
| jhyy 编译器比 C 版慢很多 | 可接受; 自举意义 > 性能; v3.x 加优化 |
