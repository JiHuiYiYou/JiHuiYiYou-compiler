...
> ⚠️ **本文档已被取代，仅作历史保留**（2026-08-05 标记）
>
> 权威路线图（最新版）请参考：
> - v0.x C 编译器演进 → [`v0.x-c-compiler-roadmap.md`](v0.x-c-compiler-roadmap.md)（v0.8 wip → v0.9 → v0.x 退役）
> - v1.x jhyy 自举 → [`v1.0-self-hosting.md`](v1.0-self-hosting.md)（v0.9 准备 + v1.0.0 启动）
> - v2.x QBE 自写 + 多目标 → [`v2.x-qbe-rewrite.md`](v2.x-qbe-rewrite.md)
> - v3.x 语言扩展 → [`v3.x-language-expansion.md`](v3.x-language-expansion.md)
>
> 旧路线图的 v0.3.x → v1.0 全程 sprint 计划跟当前 vX.Y 轴 sprint 已脱钩：
> - "Stage 0/1/2 验证法" 仍沿用，但 **Stage 0** = jhyy_v1 编 arena.jhyy 通过 QBE typecheck（v0.8 commit 12 已解锁） → **Stage 1** = diff jhyy_0.il vs jhyy_1.il byte-equal（v0.9 目标） → **Stage 2** = jhyy_1 编 jhyy_1 跑 regress 持平 baseline（v1.0 目标，**12 OK 持平即可，不是 47/47**）
> - "v0.5.0 质量门禁" 已升级为 v0.6+ 自举门槛（ABI 锁定 + lang-spec 锁定 + 9/11.1 ABI 阻塞问题已修 7/11）
> - "v1.1 / v1.2 后续" 拆为 v1.x 5 sprint + v2.x N 代 fixed point + v3.x 14 OS-required/language 特性
>
> 不要按此 doc 排期；用权威 doc。
