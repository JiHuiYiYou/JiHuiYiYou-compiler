...
| [`v2.9.0-plan.md`](v2.9.0-plan.md) | **sprint 计划**（V2-C Part 1, 待 ship）| v2.9.0 = N≥3 selfhost fixed point 验算 + verification harness（`fixed_point.sh` + `fixed_point_summary.py` + cap_test.jhyy 跨代一致）。~290 LOC 1 commit。**M5 deferral 第二前置 Part 1**（verification 闭环）|
