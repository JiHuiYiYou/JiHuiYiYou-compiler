...
| [`v2.11.3-plan.md`](v2.11.3-plan.md) | **sprint 计划**（V2-C Part 2a-后-补, 待 ship, 串行 v2.11.2 + v2.12.0 之间）| v2.11.3 = W-074.7 self-backend EXIT exact (Tier 4 全 + per-function frame table + nested_struct_deep struct-offset paths + big_test bring-up + Rule 3 重启 size-suffix gate + unique label counter + slots 4096→16384) + scope DOWN ship gate (5/5 QBE PASS + 5/5 self-backend EXIT exact match)。~280 LOC 2 commits。**M5 deferral 第二前置 Part 2a-后-补** (W-074.6 全闭环 → v2.12.0 QBE 移除 可 ship)。|
