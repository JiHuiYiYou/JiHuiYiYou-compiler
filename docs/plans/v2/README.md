...
| [`v2.11.0-plan.md`](v2.11.0-plan.md) | **sprint 计划**（V2-C Part 2a, 待 ship, 串行 v2.9.0 + v2.12.0 之间）| v2.11.0 = codegen_amd64_run 真修 (W-073 verification gap closure + W-074 0-byte .s 根因定位 + regress.py `--self-backend` flag + dual-path 5/5 self-backend + 5/5 QBE PASS)。~140 LOC 2 commits。**M5 deferral 第二前置 Part 2a**（self-backend 真能用 → v2.12.0 QBE 移除 prerequisite 解锁）|
| [`v2.12.0-plan.md`](v2.12.0-plan.md) | **sprint 计划**（V2-C Part 2b+3, 待 ship, 串行 v2.11.0 后, renumbered from v2.10.0 per 2026-09-09 user 决定）| v2.12.0 = QBE 工具链完全移除 + toolchain self-contained closure（`run_qbe` 留空 stub + `qbe.exe` 删 + `qbe_wrapper.{c,h}` 删 + 自写后端 fatal-on-fail + Procmon 验证 0 qbe spawn）。Source 暂存 `/tmp/v2.12.0-source.patch` (254 行),v2.11.0 ship 后 reapplied。~320 LOC 2 commits。**M5 deferral 第二前置 Part 2b**（QBE 移除 → M5 可独立 sprint 启动）|
