...
| [`v2.11.1-plan.md`](v2.11.1-plan.md) | **sprint 计划**（V2-C Part 2a-补, ✅ shipped 2026-09-10 on axis-v2 + tag v2.11.1, 串行 v2.11.0 + v2.12.0 之间）| v2.11.1 = W-074.5 lexer gap closure + 7 真修延伸 (dbgfile/dbgloc/{/} + ILTOK_DIRECTIVE + parse_and_emit noop + csltw lexer + next_token_call args consume + struct offset 真修 + emit cast 真修) + scope DOWN ship gate (5/5 QBE PASS + 1/5 self-backend hello PASS per [[feedback_codegen_amd64_multifn]])。~390 LOC 3 commits (tag v2.11.0 + source + docs)。**M5 deferral 第二前置 Part 2a-补** (lexer gap closure 闭 → W-074.6 multi-func self-backend 留 v2.x 中期 per [[feedback_codegen_amd64_run_zerobyte]])。|
| [`v2.12.0-plan.md`](v2.12.0-plan.md) | **sprint 计划**（V2-C Part 2b+3, 待 ship, 串行 v2.11.1 后, renumbered from v2.10.0 per 2026-09-09 user 决定）| v2.12.0 = QBE 工具链完全移除 + toolchain self-contained closure（`run_qbe` 留空 stub + `qbe.exe` 删 + `qbe_wrapper.{c,h}` 删 + 自写后端 fatal-on-fail + Procmon 验证 0 qbe spawn）。Source 暂存 `/tmp/v2.12.0-source.patch` (254 行),v2.11.0 ship 后 reapplied。~320 LOC 2 commits。**M5 deferral 第二前置 Part 2b**（QBE 移除 → M5 可独立 sprint 启动）。**v2.12.0 ship gate 调整待 user 决定**:(A) 等 v2.x 中期 W-074.6 真修后 ship 5/5 self-backend PASS;(B) 跟 v2.11.1 一样 scope DOWN 5/5 QBE + 1/5 self-backend hello PASS。|
