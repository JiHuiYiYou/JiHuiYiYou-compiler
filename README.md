...
> **v2.13.10 ship 2026-09-22 on axis-v2 + tag `v2.13.10`** + **main mirror commit**: 🎯 **W-082 audit-flip docs-only + convention enforcement (跟 v2.13.9 W-081 pattern)** (~30-45 LOC total; LOW risk)。v2.13.9 ship 后剩 1 DEFERRED: W-082 (QBE-side codegen.jhyy short-circuit `&&`/`||` nested if-condition phi merge gap, NEW in v2.13.8 audit)。**Phase 1 verification 关键发现** (per Plan agent analysis 2026-09-22):(1) user-facing `&&`/`||` 不会触发 W-082 — `cg_expr` 在 line 2112-2153 处理 `TOKEN_AMPAMP`/`TOKEN_PIPEPIPE` 走 **store/load 模式** (`ir_emit_alloc result_slot` + `cg_emit_store_primitive` per branch + `cg_emit_load` 在 `merge_b`), **0 phi emit**; (2) W-082 trigger 只在 codegen.jhyy 内部 if-condition 写 `A && (B || C)` 形态才会触发 QBE IL "predecessors not matched in phi" 错误 (e.g. W-058 v2.13.8 作者写 `if d_op == TOKEN_PERCENT() && (op_qt == QBE_D() || op_qt == QBE_S())` 时触发); (3) workaround 已落地 codegen.jhyy:2267-2308 (W-058 v2.13.8 ship) 改用 nested plain `if` flag dispatch (per W-077 v2.11.11 已建立的 convention); (4) line 1003-1005 + 2103-2110 comments 显式记录 "v0 codegen bug 2 workaround: 避免 &&/|| 嵌套 short-circuit"。**Option A 真修** (block stack + phi predecessor real-time sync, ~50-80 LOC source) 触及 phi infra = W-082 RCA 描述的 stale 区域, HIGH risk 可能 break selfhost closure (`v1→v2→v3 byte-equal` 需要 N=3..5 全跑), ROI 差 (user-facing 0 触发) → **YAGNI per user 2026-09-22 决定**, 走 audit-flip docs-only + convention enforcement。**6 files touched**: `docs/internal/workarounds.md` W-082 entry 翻 DEFERRED → "audit-flip + convention enforced per v2.13.10 mini; 真修 deferred YAGNI" + 索引行 (line 158) 同步 flip + `compiler/src0/codegen.jhyy` line 1003-1005 + 2103-2110 comment 补强 (加 W-082 reference, 纯 comment 不动 emit logic, ~3 LOC) + `scripts/dev/v2_13_10_codegen_cond_no_nested_sc.py` NEW (~50 LOC, grep `if .*&&.*\|\||if .*\|\|.*&&` in codegen.jhyy, exit 0 = convention HOLD, exit 1 = violation, exit 2 = file not found) + `docs/plans/v2/v2.13.10-plan.md` NEW (~250 LOC) + `docs/logs/v2/changelog-v2.13.0.md` v2.13.10 section append (~30 LOC) + `README.md` v2.13.10 row (本 entry)。**main mirror commit** 同步 6 files (per `0cadfba` precedent; exclude src0/ 真改 = 0 src change 因 codegen.jhyy 只 comment 补强, 不算 logic 改)。**诚实记录 per [[feedback_fix_evaluation_rule]]**: **V.3 convention grep exit 0** (post-W-058 v2.13.8 ship baseline = 0 hit, convention 已守住); regress baseline HOLD 强制 (V.0/V.1); match.jhyy EXIT=20 双 backend 一致 硬 STOP #3 trigger preserved; byte-equal D26 5/5 PASS preserved; byte-equal-amd64 V2-B 10/10 PASS preserved; fixed-point N=3..5 closure preserved (audit-flip docs-only 不动 codegen emit 主路径)。**scope DOWN vs 原 W-082 entry 估** (~50-80 LOC 真修) → 0 src change + convention enforced (跟 v2.13.9 W-081 同 pattern, 跟 W-074.8 v2.13.2 ship precedent 一致)。**Pre-existing plan drift check** (per [[feedback_doc_refactor_factcheck]]): v2.13.9 plan line 36-38 估的 "vendor QBE 升级 / W-074.6 family / W-074.7.9 → v2.13.11+/12+/13+ mini" — **三个都 RESOLVED** (vendor QBE 不在 backlog, W-074.6 family 已 v2.13.0 ship FULL CLOSED per workarounds.md:147, W-074.7.9 已 v2.13.1 audit-flip CLOSED per workarounds.md:151), 不在 backlog; 真实 backlog 只有 W-082 (本 sprint) + self-backend fmod_f32 + fmod_negative (v2.13.11 mini)。**next**: v2.13.11 mini = self-backend fmod_f32 + fmod_negative 真修 (codegen_amd64_emit_call.jhyy:1834 `is_rem` branch 加浮点分支 libm `fmod()` call wrap + x86-64 sequence, ~30-60 LOC, per user 2026-09-22 决定); 后续 vendor QBE 升级 / M5 启动前置 / QBE 自写 + QBE removal 仍推后续 sprint (不在 backlog 直接依赖 W-082 closure)。详见 [`docs/plans/v2/v2.13.10-plan.md`](docs/plans/v2/v2.13.10-plan.md) + [`docs/logs/v2/changelog-v2.13.0.md`](docs/logs/v2/changelog-v2.13.0.md) § v2.13.10 + `docs/internal/workarounds.md` W-082 audit-flip closure 段 + `scripts/dev/v2_13_10_codegen_cond_no_nested_sc.py` (convention enforcement regression script)。

> **v2.13.11 ship 2026-09-22 on axis-v2 + tag `v2.13.11`** + **main mirror commit**: 🎯 **self-backend `emit_conv_swtof` 2 bugs 真修 (W-083 NEW, W-058 self-backend path closure)** (~10 LOC source + ~400 docs = ~410 LOC; LOW risk)。v2.13.10 ship 后 user 截图 regress 发现 self-backend column 3 fmod 测试 FAIL (v2.13.10 plan line 36 错 fix 假说 "codegen_amd64_emit_call.jhyy:1834 `is_rem` branch 加浮点分支" — line 1834 不存在, file 624 行, 实际真根因在 `compiler/src0/codegen_amd64_emit_sse.jhyy emit_conv_swtof`)。**Phase 1 re-RCA** (per `feedback_rca_first_root_cause` + Read 工具验证 lexer + parse_and_emit + emit_conv_swtof 三层) 发现 2 个独立 bug: **Bug 1** = `emit_conv_swtof` hardcode `cvtsi2sd + movsd`, 不查 `tok.qbe_type`; `=s swtof` 应 emit `cvtsi2ss + movss`, 实际 emit f64 序列 → `mulss` 拿 f64 bits 解释 = garbage, `fmod_f32.jhyy` exit=7。**Bug 2** = `movl ... %eax → cvtsi2sd %rax` 之间无 sign-extension; `movl` 只清低 32-bit, upper 32-bit 是上一次 `%rax` 操作的 stale bits, 负数 i32 → huge negative → `fmod_negative.jhyy` exit=100。**Plan agent 误诊** "lexer gap" — lexer 已识别 since v2.11.19 (codegen_amd64_lexer.jhyy line 1418-1791, 8 函数 ~370 LOC) + parse_and_emit dispatch, `emit_conv_swtof` 是唯一缺口。**真修内容** (~10 LOC, 1 函数): `let is_f32: i32 = if (*t).qbe_type == QBE_S_LOCAL() { 1 as i32 } else { 0 as i32 };` + `let cvt_suffix: *u8 = if is_f32 != (0 as i32) { "ss" as *u8 } else { "sd" as *u8 };`, 全部 `cvtsi2sd`/`movsd`/`cvtsi2ss`/`movss` 拼接用 `cvt_suffix` 替代 hardcode; 在 `movl ... %eax` 后 emit `\tcltq\n` (32→64 sign-extend, 清零 upper 32-bit 至 sign-bit)。**W-058 entry line 4082 stale text 同步 fix** (per `feedback_doc_refactor_factcheck`): 删除原 "self-backend 不需额外修" 错描述 + 错 `codegen_amd64_emit_call.jhyy:1834 is_rem` 引用, 替换成 v2.13.11 真修 closure (W-083 NEW)。**5 files touched**: `compiler/src0/codegen_amd64_emit_sse.jhyy` `emit_conv_swtof` 函数体改 ~10 LOC (is_f32 + cvt_suffix + cltq, 不删任何 line) + `docs/internal/workarounds.md` W-083 NEW entry (~80 LOC) + W-058 entry line 4082 fix (~20 LOC) + 索引行 (line 163, ~1 LOC) + W-058 status 行 (~2 LOC) + `docs/plans/v2/v2.13.11-plan.md` NEW (~250 LOC) + `docs/logs/v2/changelog-v2.13.0.md` v2.13.11 section append (~50 LOC, per `feedback_changelog_umbrella` v2.13.x single umbrella) + `README.md` v2.13.11 row (本 entry)。**诚实记录 per [[feedback_fix_evaluation_rule]]** (V.0-V.8 全绿): **regress default QBE 126/147 PASS HOLD); **regress self-backend 127/127 PASS** (was 124/127 with 3 fmod fail, +3 flip → 127/127 PASS); **3 fmod tests flip** `fmod_basic` exit=1 (preserved) / `fmod_f32` exit=1 (was 7) / `fmod_negative` exit=99 (was 100); **byte-equal D26 5/5 PASS preserved**; **byte-equal-amd64 V2-B 10/10 PASS preserved**; **big_test self-backend EXIT=12345 preserved**; **fixed-point N=3 PASS + N=4/N=5 informational**; **D43 closure HOLD**; **jhyy.exe sha refresh**。**main mirror commit** 同步 5 files (per `0cadfba` precedent; exclude jhyy.exe binary)。**scope minimisation**: emit_binop (codegen_amd64_emit_call.jhyy:451-547) 0 改动 + lexer 0 改动 + parse_and_emit 0 改动 + fold IL 0 改动, 仅 1 函数 emit_conv_swtof ~10 LOC 改, 比原 plan Option (a) qt-dispatch top-level + emit_binop_sse 新 helper ~50-80 LOC 范围窄得多。**next**: v2.13.12+ mini = vendor QBE 升级 (拉 remd/rems 主线, 若支持则 fold IL 跟 native byte-equal, 零迁移成本); M5 启动前置 (jhyy 编 jhyy 0 C 依赖闭环, 独立 sprint); QBE 自写 + QBE removal (v2.x 末); ACTIVE workaround count → 0 (W-058 + W-083 双 closure = self-backend fmod 路径 0 GAP)。详见 [`docs/plans/v2/v2.13.11-plan.md`](docs/plans/v2/v2.13.11-plan.md) + [`docs/logs/v2/changelog-v2.13.0.md`](docs/logs/v2/changelog-v2.13.0.md) § v2.13.11 + `docs/internal/workarounds.md` W-083 ✅ RESOLVED entry + W-058 entry line 4082 fix + `compiler/src0/codegen_amd64_emit_sse.jhyy` `emit_conv_swtof` 函数体 (cltq + cvt_suffix 真修点)。

> **v2.14.0 ship 2026-09-22 on axis-v2 + tag `v2.14.0`** + **main mirror commit**: 🎯 **N≥10 fixed point verification + mutation testing protocol + Linux cross-platform + D43 long-term hold** (~1166 LOC source/scripts + ~211 LOC docs = ~1377 LOC total; LOW risk, **verification infra expansion = 0 src0 change**)。v2.13.11 ship 后 ACTIVE workaround count = 0 (W-058 + W-083 双 closure); v2.x 中期 M2 5 sub-sprint chain (v2.12.0 → v2.13.0 → v2.14.0) 第 3 步 (per `docs/plans/roadmap/v2.x-qbe-rewrite.md § 6.2` + `docs/plans/roadmap/v2-v3-parallel-sprint-plan.md § 5.1`)。**Phase 1 N≥10 fixed point closure** (`compiler/tests/bootstrap/fixed_point.sh` 扩 N=3 → N=10 + `JHY_FP_BASELINE_SHA` env var + `JHY_FP_FAILFAST` + per-代 timing `T_V3_BASELINE_MS=5000ms`): V.1 PASS N=10 byte-equal = `43fee332c0fdb110283a7192a26f63c6c44bb1a4c9706e1d0ba4f55400c9eb40` (per-代 timing 2.2-2.8s 全 < 1.5x baseline; fail-fast EXIT=1 verified with `JHY_FP_FAILFAST=1 JHY_FP_BASELINE_SHA=ffff...`)。**Phase 2 mutation testing protocol** (NEW `mutations.json` 286 LOC + `mutation_test.py` 426 LOC + `mutation_test.sh` 80 LOC + `mutation-test-report.md` auto-gen): 30 mutation 模板 (5 ABI + 10 codegen + 10 parser + 5 typechecker), 3 detection modes — `compile_fail` (rename → exit ≠ 0) + `compile_il_diff` (value change → exit 0 但 .il sha ≠ baseline) + `regress` (pass count drop; v2.14.0 不走 per `feedback_mcp_regress_timeout` 600s cap); ironclad try/finally + atexit + .bak 文件 restore (Belt+suspenders per `feedback_unrelated_uncommitted_revert`); V.2 PASS **30/30 = 100% catch rate, 0 false positive**, baseline regress 126/147 PASS HOLD。**Phase 3 Linux cross-platform N≥3** (NEW `fixed_point_linux.sh` 223 LOC + `d43_linux.sh` 82 LOC + docker `gcc:12` image): V.3 PASS toolchain sanity (gcc 12.5.0) + 5 main tests Win parity (hello / arith / match / cap_test / break_continue); V.4 PASS Win closure baseline = `43fee332...` HOLD + Linux gcc:12 sanity; Linux 真 self-host chain (jhyy 编 jhyy → 编 .jhyy) 等 v2.4.0 multi-target dispatcher ship 后, Phase 3 V.3 simplification per plan (avoid docker time sink)。**Phase 4 docs + ship**: `docs/logs/v2/changelog-v2.13.0.md` v2.14.0 section append (~85 LOC, per `feedback_changelog_umbrella` v2.13.x single umbrella) + `docs/logs/v2/d43-baseline-archive.md` v2.14.0 row + v2.13.11 + v2.13.7 re-baseline 记录 (~30 LOC, N=10 long-term hold) + `README.md` v2.14.0 row (本 entry) + `docs/internal/architecture.md` N≥10 + Mutation testing + Linux cross-platform 节 (~80 LOC) + `docs/internal/workarounds.md` mutation testing protocol 文档化 (~30 LOC, 新 protocol 非 workaround) + `docs/plans/v2/README.md` v2.14.0 status row (+1 LOC)。**Key insight 1 (dbgfile path cwd-sensitive, closure quirk)**: jhyy.exe 在 `(cd $JHYY_ROOT && jhyy.exe ...)` 产生 `compiler/src0/main.jhyy` 相对路径 sha `43fee332...`, 直接 invocation 产生 `C:/Users/.../main.jhyy` 绝对路径 sha `481c2e99...` — 不属 codegen bug, fixed_point.sh + d43_linux.sh 都 `cd $JHYY_ROOT` 锁住 baseline `43fee332...` (canonical 一致)。**Key insight 2 (mutation detection iterative, 60% → 93.3% → 100%)**: first run catch rate 60% (18/30) 因为 12 mutations 是常量 rename (duplicate definitions 互补, compile 通过 + .il 同), 二轮升级 `compile_il_diff` mode → 93.3% (28/30, baseline 5 mismatch), 三轮 M-002/M-003 升级 → **100% (30/30)**。**诚实记录 per [[feedback_fix_evaluation_rule]]** (V.1-V.5 全绿): **V.1 N≥10 byte-equal PASS** (10/10 byte-equal `43fee332...`, 11 代总耗时 ~28s); **V.2 mutation catch rate 100%** (30/30 caught, 0 false positive, baseline regress 126/147 PASS HOLD); **V.3 Linux 5 main tests PASS + gcc 12.5.0 toolchain sanity** (hello / arith / match / cap_test / break_continue Win-Linux parity); **V.4 D43 closure baseline HOLD** (`43fee332...`, N=10 long-term hold across v2.13.7 → v2.13.11 → v2.14.0 chain); **V.5 regress 持平** 126/147 PASS HOLD。**main mirror commit** 同步 12 files (per `0cadfba` precedent; exclude `mutation-test-report.md` auto-generated + scripts/dev/v2_14_0_*.py ship-time scripts + `compiler/tests/bootstrap/*.bak` temporary files)。**scope minimisation per [[feedback_no_artifacts_in_project]]**: src0 0 改动 (per plan v2.14.0 = verification infra expansion, NOT src0 真修); 0 src0 changes = 0 risk of regress regression + 0 risk of D43 closure break + 0 risk of jhyy.exe rebuild。**next**: v2.15.0 = QBE 自写 (跳过 QBE IL 直 emit x86-64); v2.16.0 = QBE 工具链移除 + perf bench + .exe byte-equal; v3.x OS-required (inline asm / `#[naked]` / volatile / `#[link_section]` / memory barrier / `#[no_std]` (软 ship per D10) / `&mut` + lifetime) 等 user 启动 (per `v2-v3-parallel-sprint-plan.md`); M5 启动前置 (jhyy 编 jhyy 0 C 依赖闭环) 独立 sprint; v2.14.1 docs cleanup (W-081 stale refs in `changelog v2.13.0.md:497,506` + `README.md:309,498,507,577,592`) 推后续 sprint (per `feedback_doc_refactor_factcheck`)。详见 [`docs/plans/v2/v2.14.0-plan.md`](docs/plans/v2/v2.14.0-plan.md) + [`docs/logs/v2/changelog-v2.13.0.md`](docs/logs/v2/changelog-v2.13.0.md) § v2.14.0 + [`docs/logs/v2/d43-baseline-archive.md`](docs/logs/v2/d43-baseline-archive.md) v2.14.0 row + `compiler/tests/bootstrap/mutation-test-report.md` (30 mutations 详细 log) + `compiler/tests/bootstrap/fixed_point.sh` N=10 verification + `compiler/tests/bootstrap/d43_linux.sh` Linux D43 closure。
...
> **v2.16.0 ship 2026-09-22 on axis-v2 + tag `v2.16.0`** + **main mirror commit** (1 commit ship per worktree per [[feedback_audit_single_commit_diff]]): 🎯 **QBE 工具链完全移除 + perf bench (`bench.sh`) + `.exe` byte-equal 兜底 (D26 stage0 coverage)** — **v2.x FINAL sprint**。完成 [[docs/plans/v2/v2.0.0-os-prep.md § 5.2]] v2.x 完成验收 items #7-#9:`.exe` byte-equal 兜底 (D26 stage0 coverage) + perf bench ≤ 1.1x (bench.sh first-time baseline) + C-side QBE 完全移除 (build.md "无 qbe.exe 引用")。v2.x 末 5 sub-sprint chain 第 5 步 ship 走完 (v2.12.0 → v2.13.0 → v2.14.0 → v2.15.0 → v2.16.0);**v2.x 完成验收 9 项 全 ✅**。**Phase 1 src0 + C-side backend closure** (~50 LOC net): `compiler/src0/main.jhyy` MOD `run_qbe` 改 fatal-stub (签名保留, body = "error: QBE backend removed in v2.16.0 — self-backend sole production path" + return 1;callers 链无改) + `run_backend` 简化 (删 QBE_FALLBACK 分支 + 删 BACKEND_QBE 路径) + `d26_flags` 加 `-Wl,--no-insert-timestamp`;`compiler/src0/jhyy_helpers.c` MOD 删 Layout (a)/(b) qbe.exe probing branches (L220-258) + `jh_path_qbe` 返空字符串;`compiler/src0/target_dispatch.jhyy` MOD header refresh;`compiler/src/target/target_dispatch.c` MOD SysV cases flip `BACKEND_QBE` → `BACKEND_SELF` (跟 jhyy-side 对齐 since v2.7.0)。**Phase 2 build system D26 stage0 + qbe/ removal + sha baseline** (~25 LOC net + 41 file deletions): `Makefile` 加 `CFLAGS_STAGE0 = -std=c11 -Wall -Wextra -g0 -Wl,--build-id=none -I$(SRC_DIR)` + `STRIP = strip` + `SOURCE_DATE_EPOCH = 1234567890` + stage0 rule 用 `$(CC) $(CFLAGS_STAGE0)` + `SOURCE_DATE_EPOCH` env 前缀 + post-link `$(STRIP) --strip-unneeded`;`compiler/src/main.c` 加 C-side link line `-Wl,--no-insert-timestamp` + `_putenv("SOURCE_DATE_EPOCH=1234567890")`;`qbe/` git rm -r (41 files vendored source + qbe.exe 842KB + LICENSE + Makefile + .gitignore + doc/tools/test/minic subdirs);`compiler/build/bin/jhyy.exe.sha256` baseline refresh v2.15.0 `5f225239...` → v2.16.0 `d984b9e4...` (PE/COFF TimeDateStamp PE+0x88 + build-id + COFF debug dir + link-time stamp 全锁)。**Phase 3 bench.sh + byte_equal upgrade + docs + ship** (~590 LOC across 10 files): `compiler/tests/bootstrap/bench.sh` NEW (~270 LOC, 3 programs fib(35) / ackermann(3,8) / nqueens(7) vs gcc -O2 same-source baseline, 5-run min trimmed-mean, `--report` 默认 (exit 0 accept first-time baseline) + `--strict` flag (ratio ≤ 1.1x FAIL gate,CI 用));`compiler/tests/bootstrap/byte_equal.sh` MOD [3/3] .exe byte-equal 升级 V2-only closure (V2 rebuild 4 次 byte-equal 是 FAIL gate,V1 frozen predates D26 coverage 仍 expected INFO);`docs/internal/workarounds.md` MOD W-085 NEW entry (🟡 ACTIVE) — codegen small-frame `fn(*T, i32, i32)` 第 3 i32 参数 save 寄存器错位 (frame-size threshold < 136B 时 codegen 用 `%ecx` 覆盖 `%r8d` 致调用方 arg corruption), workaround pattern: reorder signature `(i32, i32, *T)` 或加 `_pad` 让 frame ≥ 264B, bench.sh nqueens 用 reorder 已验证 nq 0.965x [PASS], fix deferred to v3.x;`docs/internal/build.md` MOD QBE 引用全删 + "v2.16.0 QBE 完全移除;自写 backend 是 sole production path" 历史注释;`docs/plans/v2/v2.16.0-plan.md` NEW (~210 LOC, per [[feedback_plans_per_version]] 每个 minor version 一个 plan file);`docs/plans/v2/README.md` MOD v2.16.0 row 📋 planned → ✅ shipped + v2.x 末 5 sub-sprint chain final-state note;`docs/logs/v2/changelog-v2.13.0.md` MOD v2.16.0 section append (~84 LOC);`docs/logs/v2/d43-baseline-archive.md` MOD v2.16.0 row;`README.md` MOD v2.16.0 row (本 entry);`installer/assets/license.rtf` + `installer/build-artifacts/jhyy-compiler-1.0.0.wxs` MOD QBE 引用删 (4 处 wxs L21/42/236/439 + license.rtf QBE IL 编译器 mention);installer 重建 deferred 到下个 release。**7 verification gates V.1-V.7 全绿** per [[feedback_fix_evaluation_rule]]:**V.1 regress 126/147 PASS HOLD** (QBE removal 不影响) + **V.2 jhyy.exe 默认走 in-mem self path, no qbe.exe spawn** (exit 42 + 0 `qbe.exe` string refs in jhyy.exe, QBE mentions only as comments/fatal-stub message) + **V.3 bench.sh fib 1.531x / ack 1.000x / nq 0.965x** (--report mode accept first-time baseline; fib 1.5x 已知 — jhyy-side recursion 没 -O2 inlining/const prop, perf opt v3.x territory) + **V.4 jhyy_stage0.exe 4/4 byte-equal sha `3d5a73ae3c00d25bdd62b698a04812f47d0d24e7ad71d6b87b1d7b84e4dca8bf`** + **V.5 jhyy.exe 4/4 byte-equal sha `d984b9e4d50bd049e0dc2ae95c47daf3119c2e5b92eaf8746d40d7ebee8ced55`** + **V.6 byte_equal.sh [3/3] V2-only closure** (V2-FAIL gate,V1 frozen INFO expected) + **V.7 ACTIVE workaround count = 1 (W-085 NEW)** + 1 commit ship per worktree。**诚实记录 W-085 discovery**:bench.sh nqueens 期间发现的 codegen small-frame `fn(*T, i32, i32)` 第 3 i32 参数 save 寄存器错位 bug (frame < 136B 时 codegen emit_call 用 `%ecx` 覆盖 `%r8d`, 致 call 端 arg corruption);bench.sh 立即 reorder signature `(i32, i32, *T)` workaround + docs 标注 + fix deferred v3.x (per [[feedback_rca_first_root_cause]] 4 个 reproducer 拆解 + assembly dump RCA 验证 + W-085 entry 文档化)。**Key insight 1 (D26 stage0 coverage 真修 chain closure)**:v2.4.0 D26 recipe `gcc -g0 + strip + SOURCE_DATE_EPOCH + --build-id=none` 已在 jhyy-side link_with_gcc 生效,但 stage0 build 没加;v2.16.0 加 CFLAGS_STAGE0 + `_putenv("SOURCE_DATE_EPOCH")` + C-side `-Wl,--no-insert-timestamp`(PE TimeDateStamp PE+0x88 的 non-deterministic byte 唯一兜底) → jhyy.exe + jhyy_stage0.exe 4/4 byte-equal verified。**Key insight 2 (run_qbe fatal-stub signature 保留 = future debug reopen friendly)**:`run_qbe` 函数签名保留 (callers in main.jhyy 不需改), body 改 fatal-stub;future debug-time reopen QBE 排查 codegen 差异只需换 body,不必改 caller chain。**Key insight 3 (vendor third-party 不算 first-party diff)**:qbe/ 41 files (~6000 LOC vendored source) 删除 + qbe.exe 842KB mirror 删除 = third-party removal, 不进 first-party diff LOC count;v2.16.0 first-party diff = +515/-551 LOC across 17 files + 1 NEW bench.sh + 1 NEW plan + 1 baseline refresh + 2 binaries rebuilt。**scope minimisation per [[feedback_no_artifacts_in_project]] + [[feedback_auto_push_after_commit]]**:src0 改 4 files (`main.jhyy` + `jhyy_helpers.c` + `target_dispatch.jhyy`) + C-side 1 file (`target_dispatch.c`) + build 1 file (`Makefile`) + `Makefile` stage0 + `main.c` D26 build + qbe/ removal + bench.sh NEW + byte_equal.sh upgrade + 8 docs = 17 files modified + 41 file deletions + 1 mirror deletion + 1 sha refresh + 2 binaries rebuilt = **1 commit ship per worktree**;main mirror commit 同步 13 files (per `0cadfba` precedent; exclude `jhyy.exe` + `jhyy.il` + `jhyy_stage0.exe` binaries + `~/.claude/plans/` + `compiler/build/bin/*.bak`)。**next**: **M5 独立 sprint** (jhyy 编 jhyy 0 C 依赖闭环 per [[docs/plans/roadmap/v1.x-phase-4-m5-boot-from-scratch.md]]) + **v3.x OS-required** (inline asm / `#[naked]` / volatile / `#[link_section]` / memory barrier / `#[no_std]` 软 ship per D10 / `&mut` + lifetime) 等 user 启动 (per [[docs/plans/roadmap/v2-v3-parallel-sprint-plan.md]]);**v2.16.0 ship = v2.x 完成定义** = v2.x 后续只有 M5 (v1.x 末 phase 4) 启动前置 + 各 milestone launch 联调支持, 不再开 v2.x 新 minor version sprint。详见 [[docs/plans/v2/v2.16.0-plan.md]] (NEW per code reality) + [[docs/logs/v2/changelog-v2.13.0.md]] § v2.16.0 + [[docs/logs/v2/d43-baseline-archive.md]] v2.16.0 row + [[docs/internal/workarounds.md]] W-085 entry + `compiler/tests/bootstrap/bench.sh` (NEW perf gate script) + `compiler/tests/bootstrap/byte_equal.sh` V2-only closure upgrade。
