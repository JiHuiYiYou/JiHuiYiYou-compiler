...
**v3.0.6/Ph.3 — src0 codegen_amd64 self-backend conversion family (W-083 Layer 1 code-only 真修) — 2026-09-23 ✅ code-only shipped / ⏳ execution proof deferred to Gate-0**

Per [`docs/plans/v3/v3.0.6-port-v2.16.0-src0-closure.md`](docs/plans/v3/v3.0.6-port-v2.16.0-src0-closure.md): axis-v3 absorbs V2.13.11 W-083 self-backend fmod 真修 (`425ab53`), 按用户 2026-09-23 "conversion 是一族, 一次补齐, 不 case-by-case" 反馈扩到 18 conv ops 一族真修 (dtosi/dtosl/dtoui/dtoul/stosi/stosl/stoui/stoul/swtof/sltof/uwtof/ultof/exts/extu/extsw/extuw/extsh/extuh/extsb/extub/truncd/truncs)。**Audit fix #2 (2026-09-23) REVERTED**: V3 当前**没有** `codegen_amd64_emit_sse.jhyy` 文件 (V3 modular split 7 files only — state/lexer/emit_mem/emit_ctrl/emit_call/regalloc/peephole 共 4594 LOC), per user "V3 modular split 不新建 SSE file" 决策 conversion 逻辑 fold 进现有 modules。4 src0 files 改: `codegen_amd64_state.jhyy` (+11 LOC, `ILTOK_CONV = 16`) + `codegen_amd64_lexer.jhyy` (+242 LOC, `next_token_conv` helper + 'd'/'s'/'u'/'e'/'t' prefix handlers 识别 18 conv ops, nested plain `if` 无 `&&`/`||` per `feedback_jhyy_brace_nesting`) + `codegen_amd64_emit_call.jhyy` (+450 LOC, `size_suffix_for_qt` 扩 "ss"/"sd" for QBE_S/QBE_D + 4 XMM/GPR scratch helpers + `emit_conv` dispatcher 18 branches 走 SSE cvttsd2si/cvttss2si/cvtsi2ss/cvtsi2sd/cltq) + `codegen_amd64.jhyy` (+5 LOC, parse_and_emit ILTOK_CONV dispatch case per W-068 fix #4 `let _ = ...` pattern)。**RCA finding (2026-09-23 Ph.3 ship 时 audit-flip closure)**: V3 self-backend `codegen_amd64_run` 对 fmod 类测试 produce 0-byte .s, root cause **多层** — **Layer 1 (W-083 真修 ✅ code-only)**: lexer 不识别 conv ops → 0-byte .s;**Layer 2 (W-086 NEW 🟡 defer)**: emit_copy / emit_binop / emit_conv **字段约定冲突** (emit_copy 读 int_val=src value, emit_binop 读 int_val=dst_id, '%' LHS handler 不写 int_val/text_len → 所有 copy 写 0, 所有 binop 写 -32(%rbp));**Layer 3 (deeper)**: lexer 不 consume operand (next_token_binop/copy 等不 consume src, lex_il 跳过未知 byte)。**Per 用户 2026-09-23 反馈 "W-083 RESOLVED 不算 (唯一能证明它的 gate ❌)"**: status flip ⏳ RESOLVED-pending-verification (从 ✅ 退回), 挂到 **Gate-0** 真跑 JHY_SELF_BACKEND=1 路径。Gate-0 ship 后灯转绿 (跟 W-086-fix 同一 logical commit group) 才正式 ✅ RESOLVED。Verification gate (revised per RCA): default QBE backend 3/3 fmod PASS ✅ (fmod_basic exit=1, fmod_negative exit=99, fmod_f32 exit=1); regress 142/142 PASS / 0 FAIL / 20 SKIP preserved ✅; `byte_equal_amd64.sh --no-strict` 5/5 PASS preserved ✅; **JHY_SELF_BACKEND=1 fmod 3/3 PASS ❌ BLOCKED by W-086** (唯一真 self-backend execution 路径, defer 真修 to Gate-0 + W-086-fix); jhyy.exe sha `95a68fd32083bf5f...` post-Phase.3 code-only rebuild。**byte_equal_amd64.sh 5/5 PASS 是 false positive** (2026-09-23 发现) — 5 tests 全走 QBE fallback (default `compile --target=amd64_win` 不设 `JHY_SELF_BACKEND` → run_backend 落 run_qbe path per `main.jhyy:823-824`), V3 self-backend 从未被任何 CI gate 真 exercise。

**v3.0.6/Gate-0 — byte_equal_selfbackend.sh NEW (造红灯) — 📋 planned**

Per 用户 2026-09-23 反馈 "在动 W-086 之前, 先让某个 gate 真正跑 JHY_SELF_BACKEND=1, 确认它红": V3 self-backend 唯一真 execution gate。`compiler/tests/bootstrap/byte_equal_selfbackend.sh` NEW (~80 LOC) 跑 `JHY_SELF_BACKEND=1 jhyy.exe compile fmod_{basic,negative,f32}.jhyy --target=amd64` + 验证 [1/4] .s 非空 (size > 100B) [2/4] gcc link success [3/4] .exe exit code 期望值 (1/99/1) [4/4] .s sha byte-equal to QBE baseline (`compiler/tests/bootstrap/baseline/fmod_*.s.sha256` 跟 byte_equal_amd64.sh QBE path 同 sha)。**Verification (this commit ship expected = 3/3 FAIL red)**: 跑 `bash compiler/tests/bootstrap/byte_equal_selfbackend.sh` → 期望 3/3 FAIL (0-byte .s + gcc link fail + exit code 异常) — **红灯是这次 ship 的全部价值所在** (给 W-086 真修一个有判定标准的 gate, 否则 W-086 修完你会在同一个假 gate 里再循环一轮)。regress 142/142 PASS preserved; byte_equal_amd64.sh 5/5 PASS preserved (不修改); jhyy.exe sha 不变。

**v3.0.6/W-086-fix — (b)+(c) one commit 真修 + Gate-0 灯转绿 — 📋 planned**

Per 用户 2026-09-23 反馈 "(b)→(c)→(a), (a) 写法不成立 (字段数根本不够), (b)+(c) 一起做": ILToken 当前 2 字段 (int_val + text_len = 16B payload) 根本装不下 binop 的 (dst, lhs, rhs) 三元组 — emit_binop 拿 int_val 当 dst_id 后, lhs/rhs 没地方放 → 塌缩成 dst_id=0 推导的固定偏移 -32(%rbp)。**正确顺序**: (b) lexer consume operand for ALL token kinds — `'%'` LHS handler 写 dst + next_token_binop consume lhs/rhs + next_token_copy consume src + next_token_conv/call 同步, **必须先做** (从"全 0"到"有真值"的开关) → (c) 扩展 ILToken 到 3-operand capacity (加 dst/lhs/rhs/op_flags + 删 L4 § 4.3 1-to-1 简化, **同一个改动的另一面**) → (a) 字段约定统一作废, 不需要单独 commit (先做 (a) 等于白做一遍)。**Files**: `codegen_amd64_state.jhyy` (~+20 LOC ILToken 扩) + `codegen_amd64_lexer.jhyy` (~+60 LOC lexer consume) + `codegen_amd64_emit_call.jhyy` (~+50 LOC emit_* 统一读 dst/lhs/rhs + 删 1-to-1) + `codegen_amd64_regalloc.jhyy` (~+20 LOC 真 src/dst temp ids, 不再 sentinel 0 fallback)。**Verification (this commit ship expected = 3/3 PASS green)**: Gate-0 灯转绿 (fmod 3/3 .s non-empty + gcc link + exit 1/99/1 + sha byte-equal to QBE baseline); regress 142/142 PASS preserved; byte_equal_amd64.sh 5/5 PASS preserved; jhyy.exe sha refresh; **W-083 ⏳ → ✅ RESOLVED** post-Gate-0-green。**Why Ph.4 must wait for this commit (per 用户反馈 "Ph.4 的验证同样会是假的")**: in-mem self-backend 之后不再落 .s 文件, Gate-0 验证依赖 .s 文件存在 + sha byte-equal 比对 — in-mem 之后 Gate-0 对不上, Ph.4 必须排在 W-086-fix 灯转绿之后。

**v3.0.7 — V3 self-backend 真 closure (W-083 + W-086 wholesale 真修 + Gate-0 灯转绿) — 2026-09-23 ✅ shipped**

Per 用户 2026-09-23 三选项确认 + "选项 4 + strict sha 锁": axis-v3 自举 self-backend 路径真 closure。v3.0.6/Ph.3 ship 时 audit-flip W-083 退回 ⏳ RESOLVED-pending-verification + W-086 NEW 🟡 DEFERRED (Layer 2/3 emit_* 字段约定冲突 + lexer 不 consume operand, 阻塞 JHY_SELF_BACKEND=1 唯一真 execution 路径, 之前所有 5/5 regress/byte_equal PASS 全是假阳性)。**3 commits**: Commit 0 `d51155e` V2 golden .s 入库 (Gate-0 红灯) + Commit 1 `git merge-file` wholesale 8 codegen_amd64_*.jhyy 跟 V2 v2.16.0 同步 (V3 modular split 命名保留) + strcmp migration (16 处 `op_name == ("X" as *u8)` → `strcmp(...) == 0`, 修 jhyy `==` on `*u8` pointer compare hang) + state cast fix (emit_mov_temp_to_xmm/emit_mov_xmm_to_temp/emit_comment 加 `let cg = state as *CGState; let s = (*cg).out; let arena = (*cg).arena;`, 修 state 是 `*CGState` NOT `*StringBuilder` hang) + cltq sign-extension (`emit_conv_swtof` 加 cltq 修 fmod_negative EXIT=100 vs 99) + 2-op form rewrite (dtosi/dtosl/dtoui/dtoul/exts 改 `cvttsd2si -N(%rbp), %eax/rax` + movl/movq, 跟 V2 v2.16.0 golden byte-equal) + ILTOK_CONV=80 (避免 ILTOK_DIRECTIVE 撞) + op_len fix (14 5-char conv ops) + emit_conv comment drop (删 `emit_comment` 在 `emit_conv` body 入口, 修 V3 vs V2 .s diff 2 行 extra) + Commit 2 W-086/W-083 ✅ RESOLVED flip + docs + tag。**Verification**: Gate-0 3/3 strict sha PASS + 3/3 exit code PASS = **6/6/0 PASS / SKIP / FAIL** (EXIT=0); fmod_basic sha=`25eccf0ad9e291f1...` byte-equal to V2 v2.16.0 golden; fmod_negative sha=`8b1d6f333eb19191...` byte-equal; fmod_f32 sha=`e6346d245ed4b6ed...` byte-equal; regress 142/142 PASS / 0 FAIL / 20 SKIP preserved (`_regress_*.exe` clean count per `feedback_regress_clean_count`); jhyy.exe sha `bc98c633ca924f1f...` post-Commit 1+2 rebuild; W-086 + W-083 status flip 🟡 DEFERRED / ⏳ RESOLVED-pending-verification → ✅ RESOLVED 2026-09-23 (workarounds.md 索引行 + body section 同步)。详尽见 [`docs/logs/v3/changelog-v3.0.md`](docs/logs/v3/changelog-v3.0.md) v3.0.7 section。
